...
class QuantumKitchenSinks():
    ...
    def __init__(self, n_episodes=1, p=None, q=None, r=1,
        ...
        self.p = 28*28 #changed from self.p = p
        self.q = 4 #changed from self.q = q
        self.r = r
        self.scale = scale
        self.distribution = distribution
        self.n_trials = n_trials
        self.num_cols = self.n_episodes * self.q
        self.num_tiles = self.q
    ...
    def _make_omega(self):
        ...
        def _create_selection_matrix():
            ...
            a = np.sqrt(self.q) #sidelength (in tiles)
            b = np.sqrt(self.p) #full image sidelength (in pixels)
            c = np.ceil(b/a) #tile sidelength (in pixels)
            for entry in range(self.p):
                
                column = np.floor(entry/b)
                row = entry%b
                tilecol = np.floor(column/c)
                tilerow = np.floor(row/c)
                m[tilecol*a+tilerow][entry] = 1
            
            #for col in range(self.p):
            #    for entry in range(a):
            #        if ((col>=entry*(b*c)) and (col<(entry+1)*(b*c))):
            #            for rownum in range(a):
            #                if np.floor(col/c)%a == rownum:
            #                    m[entry*a+rownum][col]=1
            
                #for tile_row in range(s):
                #    for tile_col in range(s):
                #        if
                #if col<(self.p/2):
                #    if (np.floor(col/14) % 2) == 0:
                #        m[0][col] = 1
                #    elif (np.floor(col/14) % 2) == 1:
                #        m[1][col] = 1
                #else:
                #    if (np.floor(col/14) % 2) == 0:
                #        m[2][col] = 1
